...
type Asset = { id: number; name: string; asset_tag: string; serial_number?: string; condition: string; location?: string; status: string; is_bookable: boolean; acquisition_cost?: string; active_allocation?: { holder_emp_id?: number; holder_dept_id?: number; expected_return_date?: string }; history: History[] };
...
export default function AssetsPage() {
  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (modal === "register") await request("/assets", { method: "POST", body: JSON.stringify({ ...data, category_id: data.category_id ? Number(data.category_id) : null, acquisition_date: data.acquisition_date || null, acquisition_cost: data.acquisition_cost || null, photo_url: data.photo_url || null, is_bookable: form.has("is_bookable") }) });
      if (modal === "allocate" && selected) await request(`/assets/${selected.id}/allocate`, { method: "POST", body: JSON.stringify({ [data.holder_type === "department" ? "holder_dept_id" : "holder_emp_id"]: Number(data.holder_id), expected_return_date: data.expected_return_date || null }) });
